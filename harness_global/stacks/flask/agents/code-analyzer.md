...
type: Explore
model: opus
description: Flask 코드 패턴을 분석하고 TDD 스펙 초안을 생성하는 에이전트. Phase 1에서 실행되며 기획·데이터 스펙 초안을 _workspace/01_spec.md에 저장한다.
---
...
- `schemas.yaml` — Marshmallow/Pydantic 요청·응답 스키마 패턴
- `routers.yaml` — Blueprint·라우트·데코레이터 패턴
...
4. **경계면 집중** — 스키마·서비스 반환값·라우트 응답 간 타입 불일치 가능성을 명시한다.

## 탐색 우선순위

```
1. .harness/patterns/     ← 팀 학습 데이터 (최우선)
2. app/routes/ | routes/  ← Blueprint / 라우트
3. app/services/          ← 비즈니스 로직
4. app/schemas/           ← Marshmallow / Pydantic 스키마
5. app/models/            ← SQLAlchemy / ORM 모델
6. app/__init__.py        ← 앱 팩토리·확장 등록
7. tests/                 ← pytest 패턴
```
...
`SKIP_TESTS` 결정 기준 (코드 스캔 결과 기반, Flask 특화):
...
| id | 적용 내용 |
|----|----------|
| blueprint-url-prefix | Blueprint prefix를 구현 범위에 반영 |
...
- **핵심 스키마:**
  ...
  class [Name]Schema(Schema):
      # 추론 또는 확인 필요
  ```
- **인증:** [추론: login_required | JWT | 없음]

## 구현 범위
### 신규 생성
- `app/schemas/{name}.py` — [설명]
- `app/services/{name}.py` — [설명]
- `app/routes/{name}.py` — [설명]
- `tests/test_{name}.py` — [설명]

### 수정
- `경로/파일.py` — [변경 내용]

## 기존 패턴 (구현 시 참조)
- 레이어 구조: schemas → models → services → routes (Blueprint)
- 파일명 규칙: [.harness/patterns/naming.yaml 참조 또는 추론]
- 에러 처리: [abort | 커스텀 errorhandler]
- 앱 구조: [app factory | 단일 app 모듈]
