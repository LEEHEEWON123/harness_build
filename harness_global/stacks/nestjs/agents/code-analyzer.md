...
type: Explore
model: opus
description: NestJS 코드 패턴을 분석하고 TDD 스펙 초안을 생성하는 에이전트. Phase 1에서 실행되며 기획·데이터 스펙 초안을 _workspace/01_spec.md에 저장한다.
---

# Code Analyzer (NestJS)
...
- `schemas.yaml` — DTO·class-validator·응답 타입 패턴
- `routers.yaml` — Controller 데코레이터·경로·가드 패턴
- `naming.yaml` — 파일명·클래스명 실제 사용 패턴
- `services.yaml` — Injectable·의존성 주입·에러 처리 패턴
...
4. **경계면 집중** — DTO·엔티티·서비스 반환값·컨트롤러 응답 간 타입 불일치 가능성을 명시한다.

## 탐색 우선순위

```
1. .harness/patterns/       ← 팀 학습 데이터 (최우선)
2. src/modules/               ← 기능 모듈 단위
3. src/controllers/           ← HTTP 엔드포인트
4. src/services/              ← 비즈니스 로직
5. src/dto/ | src/entities/   ← 입출력·엔티티
6. src/types/                 ← 공통 타입
7. test/ | src/**/*.spec.ts   ← Jest/e2e 패턴
```
...
`SKIP_TESTS` 결정 기준 (코드 스캔 결과 기반, NestJS 특화):
...
| id | 적용 내용 |
|----|----------|
| controller-guard-pattern | 가드 적용을 구현 범위에 반영 |
...
- **핵심 DTO/스키마:**
  ...
  ```
- **가드/파이프:** [추론: AuthGuard | ValidationPipe | 없음]

## 구현 범위
### 신규 생성
- `src/dto/[name].dto.ts` — [설명]
- `src/services/[name].service.ts` — [설명]
- `src/controllers/[name].controller.ts` — [설명]
- `src/[name]/[name].module.ts` — [설명]
- `src/[name]/[name].spec.ts` — [설명]

### 수정
- `경로/파일.ts` — [변경 내용]

## 기존 패턴 (구현 시 참조)
- 레이어 구조: dto → entities → services → controllers → modules
- 파일명 규칙: [.harness/patterns/naming.yaml 참조 또는 추론]
- 에러 처리: [HttpException | ExceptionFilter]
- DI 패턴: [constructor injection | 기존 모듈 exports]
...
## 주의사항
- [DTO·서비스·컨트롤러 간 타입 불일치 위험]
- [모듈 import/export 누락 위험]
```
