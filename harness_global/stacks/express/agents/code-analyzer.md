...
type: Explore
model: opus
description: Express 코드 패턴을 분석하고 TDD 스펙 초안을 생성하는 에이전트. Phase 1에서 실행되며 기획·데이터 스펙 초안을 _workspace/01_spec.md에 저장한다.
---

# Code Analyzer (Express)
...
- `schemas.yaml` — 요청/응답 검증·Zod/Joi 타입 패턴
- `routers.yaml` — Router 경로·미들웨어·핸들러 체인 패턴
- `naming.yaml` — 파일명·함수명 실제 사용 패턴
- `services.yaml` — 비즈니스 로직·DB 접근·에러 처리 패턴
...
4. **경계면 집중** — 요청 스키마·서비스 반환값·응답 JSON 간 타입 불일치 가능성을 명시한다.

## 탐색 우선순위

```
1. .harness/patterns/     ← 팀 학습 데이터 (최우선)
2. src/routes/              ← Express Router
3. src/controllers/         ← 요청 처리
4. src/services/            ← 비즈니스 로직
5. src/models/              ← DB 모델
6. src/types/ | src/schemas/ ← 타입·검증 스키마
7. test/ | __tests__/       ← Jest/Mocha/Supertest 패턴
```
...
`SKIP_TESTS` 결정 기준 (코드 스캔 결과 기반, Express 특화):
...
| id | 적용 내용 |
|----|----------|
| router-middleware-chain | 미들웨어 체인을 구현 범위에 반영 |
...
- **핵심 스키마:**
  ...
  ```
- **미들웨어:** [추론: auth | validation | 없음]

## 구현 범위
### 신규 생성
- `src/schemas/[name].ts` — [설명]
- `src/services/[name].service.ts` — [설명]
- `src/controllers/[name].controller.ts` — [설명]
- `src/routes/[name].routes.ts` — [설명]
- `test/[name].test.ts` — [설명]

### 수정
- `경로/파일.ts` — [변경 내용]

## 기존 패턴 (구현 시 참조)
- 레이어 구조: types → models → services → controllers → routes
- 파일명 규칙: [.harness/patterns/naming.yaml 참조 또는 추론]
- 에러 처리: [next(err) | 커스텀 에러 핸들러]
- async 패턴: [asyncHandler | try/catch 직접]
