---
name: dubright-dev
description: "DubRight 프론트엔드 기능 개발, 버그 수정, 컴포넌트 구현, 스토어/서비스 작업을 요청하면 반드시 이 스킬을 사용하라. Vue 컴포넌트 만들어줘, 스토어 수정해줘, 버그 고쳐줘, API 연결해줘, 페이지 추가해줘, 기능 구현해줘, 다시 실행해줘, 이전 결과 수정해줘 등 DubRight 코드 작업 요청에 모두 적용된다. 요청 앞에 'low:', 'mid:', 'high:' 레벨 키워드가 붙은 경우에도 반드시 이 스킬을 사용하라."
---

# DubRight 기능 개발 오케스트레이터

Vue 3 + Quasar + Pinia 기반 DubRight 플랫폼의 기능 개발을 3단계 파이프라인으로 조율한다.
