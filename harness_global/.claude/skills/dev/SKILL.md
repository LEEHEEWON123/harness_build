...
(레벨 키워드 명시 시에만 사용. 자동 추론 성공 시는 `[레벨 추론: ...]`, 보정 시는 `[레벨 조정: ...]` 포맷 사용)
...
| "버그", "에러", "고쳐" + 파일명·증상 특정됨 | `low` 또는 `mid` (영향 범위 1~2개 파일이면 low, 여러 파일이면 mid) |
| "버그", "에러", "고쳐" + 범위 불명확, "기능 만들어줘" + 범위 불명확 | 애매 |
...
> **`low:` 레벨 스킵:** `DEPTH_MODEL`이 `haiku`로 설정된 경우 이 Phase를 건너뛰고 Phase 2로 바로 진행한다.
