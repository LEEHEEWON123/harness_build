...
description: |
  ...
  [핸드오프 — Phase 2 구현 방식]
  커서로 핸드오프, Cursor로 구현, 여기서 구현, Claude로 구현
...
| **2** | **Claude** (기본) 또는 **Cursor** (선택) | 질문 후 분기 |
| 3, 4, 4.5, 5 | **Claude** | QA·커밋·패턴·승격 |

Phase 2에서 **먼저 사용자에게 구현 방식을 묻는다.** Cursor 핸드오프는 선택이며 기본은 Claude `implementer`다.
...
| Implementer | 2 | 스택별 레이어 구현 (기본). Cursor 핸드오프 시 미사용 |
...
### Phase 2: 구현 (Claude 기본 · Cursor 선택)

스펙 확정(Phase 1) 및 테스트 선행(Phase 1.5) 완료 후 **구현 방식을 묻는다** (필수 중단점).

#### Step 2-0: 구현 방식 선택

```
스펙 확정됐습니다. 구현을 어디서 진행할까요?

- Cursor 핸드오프: "커서" / "Cursor" / "핸드오프"
- Claude에서 계속 (기본): "여기서" / "Claude" / "ㅇㅇ" / "구현해줘"
```

| 사용자 응답 | 처리 |
|------------|------|
| `커서` / `Cursor` / `핸드오프` / `커서로` | Step 2-A → 2-B (HANDOFF, 파이프라인 중단) |
| `여기서` / `Claude` / `구현해줘` / `ㅇㅇ` / `ok` | Step 2-C (`implementer`) → Phase 3 |
| 애매함 | 한 번만 재질문 |

> **Cursor를 기본값으로 가정하지 않는다.** 명시 선택 시에만 HANDOFF.

#### Step 2-A: HANDOFF.md 작성 (Cursor 선택 시)
...
#### Step 2-B: Cursor 안내 (중단점)

```
Cursor로 핸드오프합니다.

1. Cursor에서 이 프로젝트 열기
2. "HANDOFF.md / 01_spec.md 기준으로 구현해줘"
3. 완료 후 02_implementation.md 작성, HANDOFF status → done
4. Claude Code로 돌아와 "QA 돌려줘"

워크스페이스: {WORKSPACE_DIR}
```

**파이프라인 중단.** Phase 3은 Claude 복귀 시.

#### Step 2-C: Claude 구현 (기본)
...
- `HANDOFF.md`가 있고 `status: pending|done` → Cursor 수정 안내 (FAIL 사유 포함), **implementer 재호출 안 함**
- HANDOFF 없음 → `implementer` 재호출 (최대 2회)
...
| "구현 수정해줘" | Phase 2 (Step 2-0부터: Cursor vs Claude) |
