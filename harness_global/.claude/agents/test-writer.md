...
- `test_runner`가 `auto` 또는 파일 없음 → Step 1 감지 로직 실행
- `test_runner`가 명시(vitest|jest|none) → 그 값을 사용, Step 1 감지 생략
- `test_command`가 명시되어 있으면 → `01_test_plan.md`의 "테스트 파일 명령어" 섹션에 그 명령어를 사용

---

## Step 1: 테스트 환경 감지

아래 순서로 프로젝트의 테스트 환경을 파악한다. (`test_runner: auto`이거나 config 없을 때만 실행)
