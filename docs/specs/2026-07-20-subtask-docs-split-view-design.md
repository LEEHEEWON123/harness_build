...
## 최종 결정 (2차 정정 — 라우팅 폐기, 오버레이로 전환)

Task 5까지 구현(`?issueId=&subtaskId=` 라우팅 + `IssueDocsBoard` 전용 페이지)한 뒤 사용자가 다시 정정함: **페이지 이동(라우팅) 없이, 같은 이슈 목록 페이지 안에서 오버레이로 열려야 한다.** 그리고 하위 태스크 행의 문서 진입점은 텍스트 링크가 아니라 **"문서 있음"/"문서 없음" 뱃지** 형태로, 상태에 따라 다른 스타일이어야 한다(이미 `IssueDocsBoard`의 좌측 리스트 필에서 쓴 것과 같은 뱃지 스타일).

**바뀐 것:**
- URL 쿼리(`?issueId=&subtaskId=`) 기반 라우팅 폐기. `page.tsx`는 원래(검색 파라미터 없는) 형태로 되돌린다.
- `IssueDocsBoard.tsx`(전용 페이지 컴포넌트)는 삭제하고, 그 내부 로직(리사이저·탭·저장·미리보기)을 재사용한 `SubtaskDocsOverlay.tsx`(모달형 오버레이)를 새로 만든다. 오버레이는 `issue`/`projectId` 없이 `subtasks: Subtask[]`, `initialSubtaskId: number`, `onClose`, `onSaved`만 받는다(어차피 이슈 카드가 이미 화면에 보이는 채로 열리므로 이슈 브레드크럼은 불필요).
- 오버레이 상태(`openSubtaskId`)는 `IssueSubtasks.tsx`가 직접 들고 있는다 — 이미 그 컴포넌트가 해당 이슈의 `subtasks`를 fetch해서 들고 있으므로, 별도 재조회 없이 그대로 오버레이에 넘길 수 있다.
- `IssueList.tsx`/`page.tsx`에 추가했던 `projectId` prop 전달 체인은 더 이상 필요 없으므로 전부 되돌린다(오버레이는 URL을 안 쓰므로 `projectId`가 필요 없다).
- 하위 태스크 행의 문서 진입점: `<a>` 텍스트 링크 → **뱃지 버튼**(`문서 있음`/`문서 없음`, dot+테두리+배경색, `IssueDocsBoard`의 리스트 필과 동일한 스타일)으로 교체. 클릭하면 라우팅 대신 `openSubtaskId` state를 그 하위 태스크 id로 설정해서 오버레이를 연다.
- 오버레이는 백드롭(반투명 검정) + 중앙 모달 패널, 내부 구조(좌: 그 이슈의 하위 태스크 목록, 리사이저, 우: 편집/미리보기 탭+저장)는 기존 `IssueDocsBoard`와 동일. 백드롭 클릭/Esc/×버튼으로 닫힘(미저장 변경 있으면 `confirm()`).

## 3차 정정 — 중앙 모달 → 우측 슬라이드인 패널 + 전체 너비 드래그

위 오버레이(중앙 모달)까지 구현·검증한 뒤 사용자가 다시 정정: "사이드바처럼 오른쪽에서 슬라이드되고, 화면 너비를 조절할 수 있어야 한다."

- 패널 위치: `fixed top-0 right-0 h-full`(화면 오른쪽에 고정, 전체 높이) — 중앙에 뜨는 모달이 아니라 오른쪽 가장자리에 붙는 패널.
- 슬라이드 애니메이션: 마운트 시 `translate-x-full`에서 `translate-x-0`으로 트랜지션(오른쪽 밖에서 미끄러져 들어옴).
- 전체 너비 조절: 패널 왼쪽 가장자리에 드래그 핸들을 두고, 드래그하면 패널 전체 너비(`panelWidth` state, px)가 조절된다 — 이건 패널 **내부**의 좌(하위 태스크 목록)/우(문서 편집) 스플릿용 리사이저(기존에 이미 있음, `listWidth`)와는 별개의, 패널 자체 크기 조절용 리사이저다. 최소 너비(예: 560px)와 최대 너비(뷰포트 너비 - 여백)로 clamp.
- 백드롭(반투명 검정)은 유지, 클릭 시 닫힘(미저장 변경 있으면 여전히 `confirm()`).
- 내부 좌/우 스플릿 구조, 탭, 저장, `⌘/Ctrl+S`, `Esc` 닫기는 전부 그대로 유지 — 바뀌는 건 바깥 패널의 위치/진입 애니메이션/전체 크기 조절 방식뿐.

## 결정 사항 (1차 — 이후 위 최종 결정으로 일부 대체됨)
